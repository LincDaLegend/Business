...
const getApiKey = (): string | undefined => {
    ...
    // Also check process.env.API_KEY which is defined in vite.config.ts
    return (import.meta as any)?.env?.VITE_API_KEY || (process.env as any).API_KEY;
};
...
export const analyzeBusinessData = async (data: AppState): Promise<string> => {
    try {
        ...
        const summary = {
            ...
            unpaidSalesValue: data.sales
                .filter((s) => s.paymentStatus === PaymentStatus.UNPAID)
...
export const parseSmartImport = async ({ text, files, type, totalCost }: SmartImportOptions): Promise<any[]> => {
        ...
        responseSchema = {
            ...
            items: {
                ...
                properties: {
                    ...
                    estimatedValue: {
                        type: Type.NUMBER,
                        description: "Estimated Market Value in USD (Required for weighted costing)",
                    },
    ...
    try {
        ...
        console.error("Smart Import Error details:", err?.message || err);
